...
setDT(birdStrike_df, keep.rownames = TRUE)[] # this isn't working on my machine for some reason
names(birdStrike_df)[names(birdStrike_df) == "rn"] <- "UniqueKey"
head(birdStrike_df)
```
```{r}
for (row in 1:2){
  date_time_string <- substr(birdStrike_df[row, "FlightDate"], 0, 12)
  print(date_time_string)
  date_time_parsed <- parse_date_time(date_time_string, orders = c("mdyHMS"))
  print(date_time_parsed)
...
```{r}
birdStrike_df$parsed_date <- parse_date_time(birdStrike_df$FlightDate, orders = c("mdyHMS"))
birdStrike_df
```
...
```{r}
FlightTable <- sqldf('SELECT "UniqueKey" as f_id, CAST (CAST ("FlightDate" AS VARCHAR) AS DATETIME) AS flight_date
                             , aircraft.a_id AS aircraft_id, airport.a_id AS origin_airport
