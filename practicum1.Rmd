...
## Saved a copy of birdstrikes.csv to our git repo, so no need for path anymore
...
```{r}
AirlineOperatorTable <- sqldf('SELECT "Aircraft..Airline.Operator" AS arline_name FROM birdStrike_df')
...
query <- sqlAppendTable(mydb, "State", StateData, row.names = FALSE)
...
stateDataTemp <- sqldf('SELECT DISTINCT "Airport..Name" AS airport_name, s_id AS state
                                   FROM csv
                                        JOIN stateDataFromDatabase
                                        ON "Origin.State" = state')

# Use SQLDF to query only columns needed, labeled as needed
airportData <- sqldf('SELECT DISTINCT airport_name, state
                                 FROM stateDataTemp
...
### Find Mode of number_engines for each aircraft
```{r}
...
AircraftTableEnginesOnly <- sqldf('SELECT rn AS name, most_common_engine AS number_engines FROM AircraftNumEnginesOnlyDF2')
