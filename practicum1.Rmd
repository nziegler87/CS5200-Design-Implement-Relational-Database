...
for (row in 1:nrow(birdStrike_df)){
  ...
  if (date_time_string == "UNKNOWN"){
    date_time_parsed <- as.POSIXct("1/1/1776 0:00", format="%m/%d/%Y %H:%M")
  } else {
    date_time_parsed <- as.POSIXct(date_time_string, format="%m/%d/%Y %H:%M")
  }
...
```{r}
FlightTableWithAircraft <- sqldf('SELECT "UniqueKey" as f_id, "Record.ID" AS record_id
...
FlightTableWithAircraft_AirlineOperator <- sqldf('SELECT f_id, record_id, aircraft_id, aircraft_name
                                                , airlineOperator.ao_id AS airline_operator
                                                , airlineOperator.airline_name AS airline_name
                                                , airport_name
...
FlightTableWithAircraft_Airport <- sqldf('SELECT f_id, record_id, aircraft_id, aircraft_name
                                          , airport.a_id AS airport_id
                                          , airport.airport_name AS airport_name
                                         FROM FlightTableWithAircraft_AirlineOperator
                                         JOIN AirportFromAWS AS airport
                                         ON FlightTableWithAircraft_AirlineOperator.airport_name = airport.airport_name')
FlightTableWithAircraft_Airport
```
...
BirdStrikeTable <- sqldf('SELECT "Record..ID"as birdstrike_id
                         ...
                         , "Remains.of.wilflife.collected." as wildlife_remains_collected
                         ...
                         -- I havent fully thought this join through...
                         -- we adjusted for the wildlife unknowns so match on what?
                         -- I think the ID is the same as the uniquekey...
