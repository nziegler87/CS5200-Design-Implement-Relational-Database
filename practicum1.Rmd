...
birdStrike_df = subset(birdStrike_df, select = -c(parsed_date) )
```
...
for (row in 1:nrow(birdStrike_df)){
  ...
  birdStrike_df[row, "parsed_date"] <- date_time_parsed
}
```

```{r}
sqldf('SELECT "FlightDate"
      , "parsed_date" 
      FROM birdStrike_df')
```
...
setDT(birdStrike_df_TEST, keep.rownames = TRUE)[]
...
```{r}
FlightTable <- sqldf('SELECT "UniqueKey"as f_id
                     , "parsed_date" AS flight_date
                     , aircraft.a_id AS aircraft_id
                     , airport.a_id AS origin_airport
                     ...
                     ON "Airport..Name" = airport.airport_name
...
PrecipTable <- sqldf('SELECT DISTINCT  FROM 
                  FROM birdStrike_df)')
count(PrecipTable)
