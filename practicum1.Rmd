...
authors: Nathaniel Zeigler, Danielle Tolliver
---
...
```{r}
take_off_climb_birdstrikes <- dbGetQuery(mydb, 'SELECT YEAR(flight_date) AS Year
                                                , COUNT(birdstrike_id) AS NumIncidents
                                                FROM BirdStrike
                                                JOIN Flight
                                                ON flight_id = Flight.f_id
                                                WHERE phase_of_flight IN ("Take-off run", "Climb")
                                                AND flight_date >= 2008 AND flight_date <= 2011
                                                GROUP BY Year')

landing_birdstrikes <- dbGetQuery(mydb, 'SELECT YEAR(flight_date) AS Year
                                        , COUNT(birdstrike_id) AS NumIncidents
                                        FROM BirdStrike
                                        JOIN Flight
                                        ON flight_id = Flight.f_id
                                        WHERE phase_of_flight IN ("Approach", "Descent", "Landing Roll")
                                        AND flight_date >= 2008 AND flight_date <= 2011
                                        GROUP BY Year')
...
```{r}
# installing the package to show the bar chart
library(ggplot2)
